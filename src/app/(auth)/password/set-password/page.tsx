...
export default function SetPassword() {
    ...
    async function ConfirmLink() {
        try {
            ...
            if (res.status === 200 || res.status === 201) {
                ...
                setError({status: true, message:'This link is expired, try to generate another link'});
                setTimeout(()=> {
    ...
    async function SubmitNewPassword() {
        try {
            setLoading(true);
            const response = await axios.post('https://api.webmaxi.net/api/auth/reset-password', {
                email,
    ...
    return (
       <Suspense fallback={<div> Loading...</div>}>
         <main className="w-full flex flex-col justify-center items-center gap-4 ">
            ...
            {
                success && <p className="text-green-500 text-2xl"> Password successfully changed </p>
            }
            <main className="w-full flex flex-col gap-4">
                ...
                <FilledButton loading={loading} title="Reset password" handleClick={SubmitNewPassword} />
                <BackToLogin />
            </main>
        </main>
       </Suspense>
