...
export default function Rankings() {
  ...
  const bing = route?.bing && Array.isArray(route.bing)
    ? route.bing.map((item: any) => item)
    : [];

  const google = route?.google && Array.isArray(route.google)
            ...
            <table className="py-4 overflow-x-auto  w-full">
              ...
              <tbody className="text-xs">
                {(se == "google" ? google ?? [] : bing ?? []).map((data: any, index: number) => {
                  ...
                  const gpastRank = previousRouteGoogle && previousRouteGoogle[index]
                    ? previousRouteGoogle[index].rank
                    : 0;

                  const bpastRank = previousRouteBing && previousRouteBing[index]
                      ...
                      <td className=" p-2 ">
                        <span className={`flex items-center text-xs p-1 gap-1`}>
                          ...
                          >
                            ...
                            <FaArrowUp /> {se == "google" ? data.rank !== gpastRank && (data.rank - gpastRank).toFixed(2) : data.rank !== bpastRank && (data.rank - bpastRank).toFixed(2)}
