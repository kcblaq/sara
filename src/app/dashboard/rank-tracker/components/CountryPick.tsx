...
export default function CountryPick() {
  ...
  useEffect(()=> {
    ...
    return (
        ...
        <Menu as="div" className=" w-[184px] shadow-sm h-38px  relative inline-block text-left">
    ...
    <span className="flex items-center gap-2">
        <img src={currentCountry.flags.svg} alt={currentCountry.flags.alt} className="h-5 w-5 rounded-full" />
        <b className="">{currentCountry.name.common} </b>
    </span>
) : (
    <span>Loading...</span>
)}
    <IoChevronDownOutline className="-mr-1 ml-2 h-5 w-5 text-black" aria-hidden="true" />
</Menu.Button>
          ...
          >
            <Menu.Items className="absolute z-50 max-h-[184px] overflow-auto right-0 mt-2 w-full origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
              ...
              {
               countries && countries.map((prop: Country, i: number )=> {
                  ...
                  {({ active }) => (
                    ...
                    >
                      <img src={prop.flags.svg} alt={prop.flags.alt} className='h-5 w-5 rounded-full' />
                      <span className='text-lg truncate'>{prop.name.common} </span>
...
export function CountryPickAllLocationDefault({title = 'Select location'}: Prop) {
  const [countries, setCountries] = useState<Country[]>([]);
  const [currentCountry, setCurrentCountry] = useState<Country | null>(null);
