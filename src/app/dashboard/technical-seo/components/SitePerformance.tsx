...
async function fetchData(){
    ...
    <div className="grid gap-4 w-full max-w-[390px] h-[226px] rounded-md p-6 pb-2 border ">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col items-stretch justify-end w-full text-sm text-[#475467]">
          {
            data && Object.entries(data?.pageLoadSpeed).map(([key, value], index)=> {
              return (
                <span key={index} className={`flex items-center gap-4 w-full justify-end`}> 
                <span className={` 
                ${index === 0 && "bg-[#12B76A]"} 
                ${index === 1 && "bg-[#2E90FA]"} 
                ${index === 2 && "bg-[#D1E9FF]"} 
                ${index === 3 && "bg-[#FEDF89]"} 
                h-2 w-2 rounded-full`} > </span> {key} <b>({value as number} )</b> </span>
              )
            })
          }
  ...
  return (
    isLoading ? <div className=" w-full h-20 flex items-center justify-center mt-10"> <Loader /> </div> :
      <main className="grid gap-6 mt-10">
        <section className=" grid w-full grid-cols-1 md:grid-cols-3 gap-6 2xl:gap-8 py-4 mb-20">
