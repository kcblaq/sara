...
export default function OrganicTrafficCard() {
  const { loading, error, metrics } = useSelector(
  ...
  const actualOrganicTraffic =
    metrics?.history?.scores[0]?.organic_traffic ?? 0;
  ...
  const style =
    scores?.length === 1
      ...
      : scores &&
        scores.length >= 2 &&
        scores[1]?.organic_traffic !== undefined &&
        scores[0]?.organic_traffic !== undefined
  ...
  // mock page data
  const pageData = [4, 20, 30, 35, 45];

  const per = millisecondsToSeconds(
    TrafficIncreasePercentage(
      previosUpdate?.average_time_on_site,
      actual?.average_time_on_site
    )
  )?.toPrecision(2);
  const checkPercentage = TrafficIncreasePercentage(
    previosUpdate?.organic_traffic,
    actual?.organic_traffic
  );
