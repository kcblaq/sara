...
interface Props {
  title: string, src: string, description: string, fix: string
}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col w-full">
        ...
        <div className=" text-[#475467] gap-1 flex items-center font-normal cursor-pointer w-full "
        // onClick={showDetail}> Description and how to fix 
        >
          Description and how to fix <PiCaretDown onClick={() => setOpen(!open)} />
          {open && <AutoModal closeModal={() => setOpen(false)} ModalBody={
            <div className="flex w-full  items-center flex-col p-4 gap-2">
              <span className=" font-semibold text-black"> Issue description </span>
              <hr />
              <p className="break-words hyphens-auto"> {description}</p>
              <span className=""> <Button onClick={() => setOpen(false)}> Close</Button> </span>
            </div>
          } />}
