...
export default function KeywordGap({ data, prev }: { data: KeywordGapType[], prev :KeywordGapType[] }) {
                    ...
                    <tbody>
                        {
                            data.map((item,index) => {
                                return (
                                    <tr className={`${index === data.length - 1 ? '' : 'border-b'}`} key={index}>
                                        ...
                                        <td>
                                            {ShortenNumber(item?.keyword_volume)}
                                        </td>
                                        <td className=""> 
                                            <span className={` rounded-3xl flex items-center ${item?.keyword_difficulty < prev[index]?.keyword_difficulty ?? 0 ? 'bg-[#FEF3F2] text-[#F04438]':'bg-[#F6FEF9] text-[#6CE9A6]'}`}>
                                              <TbPointFilled/>  {item?.keyword_difficulty}
                                            </span> 
                                        </td>
                                        <td>{item?.competition?.toFixed(2) ?? 0} </td>
                                        <td>{item?.rank?.toFixed(2) ?? 0} </td>
                                        <td className="bg-[#EFF8FF] pl-4">{item?.domain1?.toFixed(2)} </td>
                                        <td className="bg-[#FDF2FA] pl-4">{item?.domain2?.toFixed(2)} </td>
