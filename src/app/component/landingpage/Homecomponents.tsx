...
export function HeaderSection({ title, description, section }: HeaderSectionProp) {
    return (
        <>
...
export function FeatureCard({ title, icon, description }: FeaturecardProps) {
    return (
        <section className=" flex flex-col w-full max-w-[384px] items-center gap-5 justify-center">
            <Image src={`${icon}`} alt="Graph " height={48} width={48} />
            <div className=' flex flex-col gap-2'>
                <p className="text-center text-[#101828] text-xl font-semibold">{title} </p>
                <h2 className="text-center text-base text-[#475467] font-normal ">{description} </h2>
            </div>
        </section>
    )
}
